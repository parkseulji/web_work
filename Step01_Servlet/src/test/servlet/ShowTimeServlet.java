...
public class ShowTimeServlet extends HttpServlet{
	/*
	 * service() 메소드는 HttpServletRequest 객체의 참조값과
	 * HttpServletResponse 객체의 참조값이 전달된다.
	 * HttpServletRequest 객체는 요청한 클라이언트의 정보를 확인할 수 있는 기능이 들어있고,
	...
	protected void service(HttpServletRequest req, 
		...
		pw.println("<html>");
		pw.println("<head>");
		pw.println("<meta charset='urf-8'/>");
		pw.println("<title>현재시간 페이지</title>");
		pw.println("</head>");
		pw.println("<body>");
		//날짜 객체를 생성한다.
		Date d=new Date();
		//현재날짜 시간 정보를 문자열로 간단히 얻어내기
		String info=d.toString();
		pw.println("<p>현재시간 : <strong>"+info+"</strong></p>");
		pw.println("</body>");
		pw.println("</html>");
		pw.close();
	}
